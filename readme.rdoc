...
A file called foo.pdf with an extension pdf and the content is a pdf(with header %PDF-1.4ˆM) then your api will work file but whether the user rename foo.pdf to foo.txt your api will fail or better it will determine incompatible data with the file extension.
